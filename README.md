...
- 省份科类批次规则：维护不同年份、不同省份的科类和批次规则。
- 省控线科类批次规则：维护不同年份、不同省份的省控线科类和省控线批次规则。
...
| 省份科类批次规则 | `年份`、`省份`、`招生科类/科类`、`招生批次/批次` |
| 省控线科类批次规则 | `年份`、`省份`、`省控线科类`、`省控线批次` |
...
- 原始科类为 `物理类 / 历史类 / 文科 / 理科 / 综合` 等内容时，会结合招生年份、省份规则自动推导招生科类和首选科目。

示例：

```text
山西 + 文史类 + 2025 / 25 / 25年  => 历史类
山西 + 文史类 + 2024 / 24 / 24年  => 文科
```
...
该模块把原来的 `院校分提取（普通类）` 和 `院校分提取（艺体类）` 合并到同一个页面中。页面顶部提供 2 个标签页：
...
- `src/pages/tools/CollegeScoreTool.tsx`：院校分提取总入口，只负责顶部标签页切换。
- `src/pages/tools/CollegeScoreNormalTool.tsx`：普通类院校分提取页面和原处理逻辑，需要保留。
- `src/pages/tools/CollegeScoreArtTool.tsx`：艺体类院校分提取页面和原处理逻辑，需要保留。

`CollegeScoreTool.tsx` 依赖 `CollegeScoreNormalTool.tsx` 和 `CollegeScoreArtTool.tsx` 渲染两个标签页内容，因此这两个文件不能删除。这样做可以只合并入口和页面展示，不改普通类、艺体类原有处理逻辑。
...
- 招生人数、录取人数按组求和。
- 平均分字段不填内容。
- 院校招生代码后、数据是否有问题前保留 `层次` 字段。
- 专业组代码、招生代码按文本格式导出，并去掉开头的 `^`。
- 省控线科类、省控线批次按规则中心对应年份规则自动填充；特殊省份或特殊批次按模块原规则处理。
...
用途：将“库中导出专业分模板”转换为“专业分导入模板”，不新增左侧独立工具入口，直接放在 `模版转换工具` 页面内。

输入文件表头要求：

```text
年份、省份、学校、科类、批次、招生类型、专业、层次、备注、最高分、平均分、最低分、最低分位次、招生人数、录取人数、专业组代码、专业组选科要求、专业选科要求(新高考专业省份)、招生代码、专业代码、数据来源
```

源文件中如果有 `方向` 字段，会写入导出模板的 `专业方向（选填）`。

主要转换规则：

- 源文件 `年份` 写入导出模板 `B2`；`A2` 固定为 `招生年份`。
- 导出模板 `A1:U1` 合并，字体为红色，行高为 350 磅。
- 第 3 行输出专业分导入模板字段。
- 源文件 `学校` 写入 `学校名称`；`专业` 写入 `招生专业`；`备注` 写入 `专业备注（选填）`。
- 源文件 `科类` 写入 `招生科类`；仅当科类为 `物理类 / 历史类` 时，`首选科目` 写入 `物 / 历`，其他科类不填首选科目。
- 新疆、西藏按老高考处理，不填写 `首选科目`、`选科要求`、`次选科目`。
- 源文件 `专业组选科要求` 与 `专业选科要求(新高考专业省份)` 会合并解析后写入 `选科要求` 和 `次选科目`。
- 源文件 `专业组代码`、`招生代码`、`专业代码` 会先去掉 `^`，再按文本格式导出。
- 源文件 `层次` 为 `专科` 时，导出为 `专科(高职)`。
...
- 选择 Excel 文件和 Sheet。
- 选择备注字段；系统会优先自动选择 `专业备注`、`备注` 或字段名中包含“备注”的列。
- 按关键词和优先级提取招生类型。
- 备注中包含需要核查关键词时，标记 `需要核查 = 是`。
- 命中需要核查关键词时，不再提取招生类型，避免把否定语境误判为招生类型。
- 导出字段：`备注`、`招生类型`、`需要核查`、`命中核查关键词`。
...
用途：单独复用学业桥专业分中的备注清洗逻辑，对任意 Excel 中的备注字段进行批量处理。

使用方式：

1. 上传 Excel 文件。
2. 选择 Sheet。
3. 选择备注字段；系统会优先自动选择 `专业备注`、`备注` 或字段名中包含“备注”的列。
4. 点击 `开始处理备注`。
5. 预览 `原始备注`、`修改后的备注`、`处理结果`。
6. 点击 `导出备注处理结果`。
...
用途：把已经转成 PDF 的高考真题文档，按题号批量截图并导出 PNG 图片包。

使用方式：

1. 先在 Word / WPS 中把原始 DOCX 另存为 PDF。
2. 上传 PDF 文件。
3. 系统自动识别顶层题号，例如 `1.`、`1．`、`1、`。
4. 系统自动排除 `一、选择题`、`二、非选择题`、试卷标题、注意事项、单独科目名等公共内容。
5. 在“题目截图计划”中预览单题效果。
6. 点击 `导出全部题目截图 ZIP`，批量导出所有题目 PNG。

处理规则：

- 只识别顶层题号，不把小题号 `（1）`、`（2）` 识别为新题。
- 下一道题开始位置会作为上一题结束边界。
- 如果两题之间出现大标题，会优先把大标题作为上一题结束边界，避免把大标题截入上一题。
- 跨页题目会自动把多个页面片段拼接为一张图片。
- 导出的 ZIP 中会附带 `_detected_questions.txt`，用于核对题号和大标题识别情况。

可调参数：

| 参数 | 作用 |
|---|---|
| 清晰度 | 控制导出 PNG 清晰度，默认 240 DPI |
| 图片四周白边 | 控制每张题图四周留白，默认 1 cm |
| 跨页拼接间隔 | 控制跨页题上下片段之间的间隔 |
| 边界前收缩 | 如果截到下一题第一行，可适当调大 |
| 忽略页眉高度 | 如果跨页题截到页眉，可填写页眉高度 |
| 忽略页脚高度 | 如果跨页题截到页码或页脚，可填写页脚高度 |
| 拆分排除行 | 遇到注意事项、试卷标题等排除行时，拆开裁剪片段，默认开启 |

限制说明：

- 适合文档版 PDF。
- 纯扫描图片 PDF 无文本层，通常无法自动识别题号。
- 浏览器端处理大 PDF 会占用较多内存，建议先用默认 240 DPI；如果文件过大，可降低清晰度。
...
│  ├─ modules/                        # 核心数据处理逻辑
│  │  ├─ collegeScoreNormal.ts        # 普通类院校分提取逻辑
│  │  ├─ collegeScoreArt.ts           # 艺体类院校分提取逻辑
│  │  ├─ controlLine.ts
│  │  ├─ employmentReport.ts
│  │  ├─ groupCodeMatch.ts
│  │  ├─ libraryProfessionalScoreTemplate.ts
│  │  ├─ manualMatchHint.ts
│  │  ├─ match.ts
│  │  ├─ planCompare.ts
│  │  ├─ questionScreenshot.ts
│  │  ├─ remarkTypeExtract.ts
│  │  ├─ ruleCenterValidation.ts
│  │  ├─ segmentation.ts
│  │  ├─ segmentationParsers.ts
│  │  ├─ standardize.ts
│  │  ├─ templateExport.ts
│  │  ├─ transform.ts
│  │  ├─ uploadValidation.ts
...
│  │  ├─ ExportStep.tsx
│  │  └─ tools/
│  │     ├─ CollegeScoreTool.tsx       # 院校分提取合并入口，负责普通类/艺体类 Tab
│  │     ├─ CollegeScoreNormalTool.tsx # 普通类院校分提取页面，保留
│  │     ├─ CollegeScoreArtTool.tsx    # 艺体类院校分提取页面，保留
│  │     ├─ XueyeqiaoTool.tsx
│  │     ├─ GroupCodeMatchTool.tsx
│  │     ├─ PlanCompareTool.tsx
│  │     ├─ EmploymentReportImageTool.tsx
│  │     ├─ RemarkTypeExtractTool.tsx
│  │     ├─ QuestionScreenshotTool.tsx
│  │     └─ SegmentationCheckTool.tsx
│  ├─ services/                       # Firebase 服务封装
│  │  ├─ firebaseAuth.ts
│  │  └─ ruleCenterFirebaseService.ts
│  ├─ stores/                         # Zustand 状态
│  ├─ styles/                         # 主题和动画样式
│  ├─ types/                          # 类型定义
...
1. **Excel 表头位置要符合对应工具要求**  
   不同工具读取表头的行不同，例如专业分智能填充默认读取第一行，院校分提取读取模板固定行，专业组代码匹配的专业分导入模板从第 3 行后读取数据。
...
4. **招生年份会影响科类和批次规则**  
   专业分模板智能填充、院校分提取、招生计划数据比对等涉及科类/批次规则的功能，会根据年份读取规则中心对应年份规则。

5. **院校分提取的普通类和艺体类逻辑仍分别维护**  
   左侧入口已合并为 `院校分提取`，但普通类、艺体类仍分别由 `CollegeScoreNormalTool.tsx` 和 `CollegeScoreArtTool.tsx` 承载，避免合并入口时影响原有处理流程。

6. **导出前先处理异常记录**  
   建议导出前确认 `error`、`warning` 级别问题，避免异常数据进入最终模板。

7. **规则中心影响多个工具**  
   学校规则、专业规则、备注招生类型规则、需要核查关键词、科类批次规则会被多个页面复用。修改规则后，建议重新处理对应文件。
