...
import {
  exportLibraryProfessionalScoreWorkbook,
  processLibraryProfessionalScoreRows,
...
function buildLibraryRowKey(row: LibraryProfessionalScorePreviewRow) {
  return [
    row.rowId || '',
    row.招生年份 || '',
    row.学校名称 || '',
...
const LIBRARY_TEMPLATE_TABLE_COLUMNS = [
  { title: '招生年份', dataIndex: '招生年份', key: '招生年份', width: 100 },
  { title: '学校名称', dataIndex: '学校名称', key: '学校名称', width: 180 },
  { title: '省份', dataIndex: '省份', key: '省份', width: 100 },
  { title: '招生专业', dataIndex: '招生专业', key: '招生专业', width: 180 },
  { title: '专业方向', dataIndex: '专业方向（选填）', key: '专业方向（选填）', width: 160 },
  { title: '专业备注', dataIndex: '专业备注（选填）', key: '专业备注（选填）', width: 220 },
...
function XueyeqiaoConvertPanel() {
  ...
  const previewRows = useMemo(() => {
    return (result?.previewRows || []) as XueyeqiaoPreviewRow[]
  }, [result])
...
function LibraryTemplateConvertPanel() {
  ...
  const handleExport = async () => {
    ...
    try {
      const blob = await exportLibraryProfessionalScoreWorkbook({
        exportRows: result.exportRows,
        yearValue: result.yearValue,
      })
